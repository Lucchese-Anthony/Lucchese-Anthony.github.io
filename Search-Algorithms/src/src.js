...
function changeGraph() {
    ...
    var c = confirm("are you sure you want to change the graph?")
    if (!c) {
        return;
    }
    ...
    if (graph == "grid") {
        ...
        s.graph.clear();
        customGraph = customGraph.replace(/'/g, '"');
    ...
    s.refresh();
    document.getElementById("start").placeholder = parseInt(s.graph.nodes()[0].id + 1);
    document.getElementById("end").placeholder = parseInt(s.graph.nodes()[s.graph.nodes().length - 1].id) + 1;
...
function getNeighbors(node) {
  ...
  for (var i = 0; i < s.graph.edges().length; i++) {
    if (s.graph.edges()[i].source == node.id || s.graph.edges()[i].source == node.id) {
